...
class InpaintCAModel(Model):
    ...
    def build_gconv_inpaint_net(self, x, mask, config=None, reuse=False,
                                training=True, padding='SAME', name='inpaint_net'):
        """Inpaint network(with gated convolutional layers).
    ...
    def build_server_graph(self, batch_data, reuse=False, is_training=False):
        ...
        if self.version == 'v1':
            x1, x2, flow = self.build_inpaint_net(
                ...
                config=None)
        elif self.version == 'gconv':
            x1, x2, flow = self.build_gconv_inpaint_net(
                batch_incomplete, masks, reuse=reuse, training=is_training,
                config=None)
        elif self.version == 'v2':
            x1, x2, flow = self.build_v2_inpaint_net( 
                batch_incomplete, masks, reuse=reuse, training=is_training,
                config=None)
        batch_predict = x2
        # apply mask and reconstruct
